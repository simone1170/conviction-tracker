# Conviction Tracker — Full Specification (v2)

## 1. Core Philosophy

Track the objective financial commitments of "smart money" (corporate insiders, C-suite, Congress members). If insiders are buying heavily during a dip, that is your entry signal. The system follows money, not media — it scores conviction by combining who is buying, how much, and whether the signal clusters across a sector.
...
- **Exclusions:** Ignore transactions flagged as 10b5-1 planned trades (see §5.4)
- **Alert:** Immediate Telegram push — always instant, never batched
- **Confidence boost:** Inner Ring signals start at base confidence 60 and add bonuses per §6
...
- Format:
  ...
  ```
- **Trigger:** 3+ distinct insider purchases from different companies in the same sector within a 7-day rolling window
...
- **Exclusions:** Same-company buys don't count toward the cluster threshold. 10b5-1 planned trades are excluded.
...
- **Trigger conditions** (any one is sufficient):
  ...
  2. An insider purchase exceeds 5x the rolling 90-day average purchase size for that ticker (requires sufficient historical baseline — see §5.5)
  3. A first-time purchase by a politician in a sector they have never previously traded
...
### 3.1 SEC EDGAR (Form 4 — Corporate Insiders)
...
- **Transaction code filter (STRICT):**
  - `P` (Open Market Purchase) → **BUY signal — process**
...
### 3.2 Congressional Disclosures (STOCK Act)
...
- **Lag handling (CRITICAL):**
  - Congress members have up to 45 days to report
  - The system must treat congressional trades as **lagging macro confirmations**, not leading tactical signals
  ...
  - The `report_lag_days` field (computed: `report_date - transaction_date`) should be displayed prominently
...
CREATE TABLE trades (
    ...
    filing_url TEXT,
    is_planned_trade BOOLEAN DEFAULT FALSE,  -- 10b5-1 flag
    ring TEXT CHECK(ring IN ('inner', 'middle', 'outer')),
    confidence_score INTEGER,            -- 0-100, computed by scoring engine
    alert_sent BOOLEAN DEFAULT FALSE,
    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
    -- Deduplication constraint: same person, ticker, date, and value = same trade
...
CREATE TABLE system_health (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    component TEXT NOT NULL,             -- 'scraper_sec', 'scraper_congress', 'etf_updater', 'alerter'
    last_successful_run TEXT NOT NULL,   -- ISO 8601
...
#### politician_history (Outer Ring context)
```sql
...
### 5.2 Congressional ingestion (`src/scrapers/congress_scraper.py`)
...
4. For congressional data, pull maximum available history from the API provider
...
Every trade that passes through the Bullseye filter receives a confidence score from 0 to 100. This replaces binary "signal / no signal" with a graded conviction level.

### 6.1 Base scores by ring
- Inner Ring: 60
- Middle Ring: 40 (per individual trade in the cluster)
- Outer Ring: 50
...
|---|---|---|
| Title weight | CEO or CFO | +15 |
| Title weight | President, COO, CTO | +10 |
| Title weight | Director (board member) | +8 |
| Title weight | VP or other officer | +3 |
| Ownership type | Direct (D) | +0 (baseline) |
| Ownership type | Indirect (I) | -15 |
| Purchase size | Above 2x the ticker's 90-day avg | +10 |
| Purchase size | Above 5x the ticker's 90-day avg | +20 (replaces +10) |
| Cluster strength | 4+ companies in Middle Ring cluster | +10 |
| Cluster strength | 5+ companies in Middle Ring cluster | +20 (replaces +10) |
| Repeat buyer | Same person bought this ticker in last 180 days | +5 |
| Congressional size | Upper range bound >= $250K | +10 |
| Congressional lag | Report lag <= 15 days (unusually fast) | +5 |

### 6.3 Penalty modifiers (subtractive)

| Factor | Condition | Penalty |
|---|---|---|
| Planned trade | 10b5-1 flagged | set score to 0, exclude from alerts |
| Indirect ownership | Via trust or LLC | -15 (already in bonus table) |
| Stale congress data | Report lag > 30 days | -10 |
| Low absolute value | SEC purchase < $10,000 | -20 |
...
#### Inner Ring alert (always immediate)
```
🎯 INNER RING — {TICKER}
...
{person_name} ({source_label})
${total_value:,.0f} {purchase|sale}
...
**Problem:** During earnings season or broad selloffs, insider buying spikes and your phone gets 15+ pings in a day. Alert fatigue causes you to ignore real signals.

**Rules:**
- Inner Ring alerts are ALWAYS sent immediately, regardless of volume. These are your highest conviction.
...
- Once the daily threshold is crossed, remaining Middle/Outer/Anti-signal alerts are queued.
- At a configurable time (default: 8:00 PM local), the queued alerts are sent as a single daily digest message.
...
Last Congress scrape: {timestamp} ({records} new trades)
DB size: {size_mb} MB | Pending alerts: {count}
```

If the daemon itself fails to run, no heartbeat is sent — the absence of the message IS the alert. You notice "I didn't get my morning ping" and know to investigate.
...
- A separate function in `src/engine/anti_signal.py` runs after each ingestion cycle
...
- `scraper_congress` — expected interval: every cron cycle
...
- Sidebar: health indicators (§8.2), quick filters (date range, ring, ticker search), link to configuration
...
- **Quick filters:** Ring type, confidence threshold slider, date range, ticker search
- Each row links to the raw SEC/Congress filing URL

### 9.3 Page: Sectors (`pages/sectors.py`)

- **Sector heatmap:** Grid showing insider buying intensity by sector (Plotly heatmap)
...
- **Columns:** Date, ticker, person, title, type, value, source, confidence, ring, 10b5-1 flag, filing link
- **Filters:** Source (SEC/Senate/House), transaction type (buy/sell), date range, min value
...
- **Database stats:** Total trades, trades by source, DB file size, oldest/newest record
...
# Congressional API
CONGRESS_API_PROVIDER=quiverquant
...
- Implement `src/config.py` (loads `.env`, defines constants)
...
- Implement `src/engine/scoring.py` (confidence scoring)
...
### Phase 5: Congressional Data + Outer Ring (2 sessions)
- Implement `src/scrapers/congress_scraper.py` (API integration)
- Implement amount range parsing
- Implement `src/engine/outer_ring.py` (anomaly rules)
- Implement `politician_history` tracking for first-time sector detection
- Write tests: mock congressional data → anomaly detection

### Phase 6: Anti-Signal Layer (1 session)
- Implement `src/engine/anti_signal.py` (sell cluster detection)
...
- Implement `pages/overview.py` (signal feed, summary cards)
...
- Implement daily digest batching logic
